...
ドット絵のマトリクスパネル上に現在時刻を表示し、パックマンとゴーストたちがその上を走り抜ける、レトロ風のデジタル時計です。純粋な HTML / CSS / JavaScript のみで動作し、ライブラリやビルド環境は不要です。
...
- **キャラクター演出** — パックマンと 4 体のゴースト（blinky / pinky / inky / clyde）が定期的に横切る
- **特別演出** — 5 回に 1 回、青いいじけゴースト 4 体が左へ逃げ、パックマンが左右反転して追いかける
...
| ファイル | 役割 |
|----------|------|
| `index.html` | 画面の骨組み。時計パネルと操作ボタンのコンテナを定義 |
...
`countDownToGroupRelease()` が一定秒ごとに `releaseTheGroup()` を呼び出します。通常回はパックマンに続いてゴースト 4 体が右へ流れます。5 回に 1 回の特別回では、いじけゴースト 4 体が左へ逃げ、そのあとをパックマンが左右反転して追いかけます。
